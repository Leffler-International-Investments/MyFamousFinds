...
export async function sendLoginCode(to: string, code: string) {
  ...
  if (!cleanedCode) throw new Error("sendLoginCode missing required field: code");
...
export async function sendSellerApplicationReceivedEmail(to: string) {
  const email = String(to || "").trim();
  if (!email) {
    throw new Error("sendSellerApplicationReceivedEmail missing required field: to");
  }
...
export async function sendAdminNewSellerApplicationEmail(
  ...
  if (!to) {
    throw new Error("sendAdminNewSellerApplicationEmail missing required field: adminTo");
  }
