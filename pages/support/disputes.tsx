...
export default function Disputes(){
  ...
  async function submit(e:React.FormEvent<HTMLFormElement>){
    ...
    await fetch("/api/disputes/create",{ method:"POST", headers:{ "Content-Type":"application/json" }, body: JSON.stringify(fd) });
    setOk(true); setBusy(false);
  }
  return (
    <>
      <Head><title>Open Dispute — Famous Finds</title></Head>
      <Header />
      <main className="wrap">
        <h1>Open a Dispute</h1>
        {ok ? <div className="ok">Submitted. We’ll email you updates.</div> : (
          <form className="form" onSubmit={submit}>
            <label>Order ID<input name="orderId" required /></label>
            <label>Role<select name="role"><option>Buyer</option><option>Seller</option></select></label>
            <label>Reason<select name="reason"><option>Not as described</option><option>Damaged</option><option>Late</option><option>Other</option></select></label>
            <label className="col2">Details<textarea name="details" rows={4} /></label>
