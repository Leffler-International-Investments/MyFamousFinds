...
export default async function handler(
  ...
  try {
    ...
    // Try to send email – but DO NOT block if SMTP is broken.
    try {
      await sendLoginCode(email, code);
    } catch (emailErr: any) {
      console.error("sendLoginCode failed (continuing anyway)", emailErr);
      // Frontend can still continue, and you can use the master code.
    }
