// FILE: /pages/management/purchases.tsx
...
type Purchase = {
  ...
  totals?: { currency?: string; total?: number; platformFee?: number; sellerPayout?: number };
  ...
  refund?: { status?: string; amount?: number; reason?: string; refundedAt?: string };
...
export default function ManagementPurchases({ purchases: initialPurchases }: Props) {
  ...
  const updateAdminCard = async (purchaseId: string, patch: Partial<AdminCard>) => {
    const current = purchases.find((x) => x.id === purchaseId);
    ...
    try {
      ...
      setPurchases((prev) =>
        prev.map((o) => (o.id === purchaseId ? { ...o, adminCard: merged } : o))
      );
    } catch (e: any) {
      console.error(e);
  ...
  const handleRefund = async (purchaseId: string) => {
    ...
    try {
      ...
      const res = await fetch("/api/orders/refund", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ orderId: purchaseId, reason }),
      });

      const json = await res.json().catch(() => ({}));
      if (!res.ok || json?.error) throw new Error(json?.error || "Failed to initiate refund");

      setPurchases((prev) =>
        prev.map((o) =>
          o.id === purchaseId
            ? { ...o, status: "Refunded", refund: { ...o.refund, status: "Refunded", reason } }
            : o
        )
      );
    } catch (e: any) {
        ...
        <main className="dashboard-main">
          <div className="dashboard-header">
            <div>
              <h1>Purchases</h1>
              <p>
                Track every purchase end-to-end: buyer, seller, item, payment method, shipping,
                delivery, signature, payout, and refund support.
              </p>
            </div>
          ...
          <div className="filters-bar">
            <input
              ...
              placeholder="Search by order ID, buyer, seller, listing title…"
          ...
          <div className="cards">
            {visible.length === 0 && <div className="empty">No purchases found.</div>}
              ...
              return (
                <div key={o.id} className={`card ${card.closed ? "closed" : ""}`}>
                  <div className="cardTop">
                    <div className="left">
                      <div className="kicker">PURCHASE</div>
                      <div className="big">{o.id}</div>
                      <div className="sub">
                        <span className={`pill ${o.status === "Refunded" ? "pill-refund" : ""}`}>
                  ...
                  <div className="rowGrid">
                    ...
                    <div className="box">
                      ...
                      <div className="value">{o.sellerName || o.sellerId || "—"}</div>
                    </div>

                    <div className="box">
                      <div className="label">Payment</div>
                      <div className="value">{o.paymentMethod || "—"}</div>
                      <div className="small">
                    ...
                    <div className="box">
                      ...
                      {ship.trackingUrl ? (
                        ...
                        <div className="small">Add tracking from this same page.</div>
                      )}
                    </div>
                  </div>

                  {/* Financial summary row */}
                  {(o.totals?.platformFee || o.totals?.sellerPayout) && (
                    <div className="financials">
                      <span>Platform fee: {money(o.totals?.platformFee, o.totals?.currency)}</span>
                      <span>Seller payout: {money(o.totals?.sellerPayout, o.totals?.currency)}</span>
                    </div>
                  )}
                  ...
                  {refund.status && (
                    <div className="refund-banner">
                      <strong>Refund:</strong> {refund.status}
                      {refund.amount ? ` — ${money(refund.amount, o.totals?.currency)}` : ""}
                    ...
                    <div className="cardBody">
                      ...
                      <div className="actions">
                        ...
                        <button
                          ...
                          onClick={() =>
                            updateAdminCard(o.id, {
                              remarks: String((purchases.find((x) => x.id === o.id)?.adminCard?.remarks || "")),
                              checklist,
                              closed: !!card.closed,
                            })
                          }
  ...
  try {
    ...
    const purchases: Purchase[] = snap.docs.map((doc) => {
      ...
      return {
        ...
        createdAt,
        paymentMethod: d.paymentMethod || d.payment?.method || "",
        paymentId: d.paymentId || d.payment?.id || "",
        totals: {
          currency: totals.currency || "USD",
          total: Number(totals.total || 0),
          platformFee: Number(totals.platformFee || totals.fee || 0),
          sellerPayout: Number(totals.sellerPayout || totals.payout || 0),
        },
