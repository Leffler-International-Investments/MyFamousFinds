...
export default function ManagementContentStaticPages() {
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    await fetch(`/api/management/content/static-pages/${page}`, {
      method: "POST",
    ...
    <>
      ...
      <main className="mx-auto max-w-4xl px-4 py-8 text-gray-900">
          ...
          {Object.keys(titles).map((k) => (
            <button
              ...
              className={`rounded-md border px-3 py-1 text-sm ${
                page === k ? "bg-gray-900 text-white" : "text-gray-700 hover:bg-gray-100"
              }`}
            >
              {titles[k]}
            </button>
        ...
        <form onSubmit={handleSubmit} className="space-y-5 rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
          ...
          <div>
            <label className="block text-xs font-medium text-gray-700">Page Content (HTML or Markdown)</label>
            <textarea
              name="body"
              rows={10}
