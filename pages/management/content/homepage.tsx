...
export default function ManagementContentHomepage() {
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    await fetch("/api/management/content/homepage", {
      method: "POST",
    ...
    <>
      ...
      <main className="mx-auto max-w-4xl px-4 py-8 text-gray-900">
        ...
        <form onSubmit={handleSubmit} className="space-y-5 rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
          <div>
            <label className="block text-xs font-medium text-gray-700">Hero Headline</label>
            <input
              name="heroTitle"
          ...
          <div>
            <label className="block text-xs font-medium text-gray-700">Hero Subheading</label>
            <textarea
              name="heroSubtitle"
              rows={2}
          ...
          <div>
            <label className="block text-xs font-medium text-gray-700">Featured Collections (comma-separated)</label>
            <input
              name="collections"
